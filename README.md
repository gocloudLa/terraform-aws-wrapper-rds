...
- 🔄 [DB Reset](#db-reset) - Deletes and recreates database for dev, supports MySQL, MariaDB, PostgreSQL
...
- **🚨 Drop/Create Database:** Drops and recreates the database within the engine - this operation is destructive and will result in data loss.
- **⚠️ Public Access:** Exposes the resource to the internet - set `publicly_accessible = true`
- **⚠️ Overwrite Database Data:** Allows overwriting database data within the engine - set `force_replace = true`
